...
class Markers
{
  ...
  public const DROP = 'DROP', BOUNCE = 'BOUNCE';
  ...
  private bool $markerClusterer = false;

  private array $clusterOptions = [];

  /**
   * Create multiple markers by array.
   * @param array $markers
   */
   ...
   * @param bool|string $animation
   * @param null|string $title
   * @return $this
   */
  public function addMarker(array $position, $animation = false, ?string $title = null): Markers
  {
    ...
    else
    {
      throw new InvalidArgumentException(
        "Animation must be string or boolean, $animation (" . gettype($animation) . ") was given"
      );
    }
   ...
   * @return $this
   */
  public function setMessage(string $message, bool $autoOpen = false): Markers
  {
   ...
   * @return $this
   */
  public function isMarkerClusterer($cluster = true): Markers
  {
    if(!is_bool($cluster))
    {
   ...
   * @return bool
   */
  public function getMarkerClusterer(): bool
  {
   ...
   * @return $this
   */
  public function setClusterOptions($options = []): Markers
  {
   ...
   */
  public function fitBounds($bound = true): Markers
  {
    if(!is_bool($bound))
    {
   ...
   */
  public function setIcon($icon): Markers
  {
    if(!count($this->markers))
    {
   ...
   * @return $this
   */
  public function setDefaultIconPath(string $defaultPath): Markers
  {
    if(!Strings::endsWith($defaultPath, '/') && !Strings::endsWith($defaultPath, '\\'))
    {
   ...
   * @return $this
   */
  public function setColor(string $color): Markers
  {
    $allowed = ['green', 'purple', 'yellow', 'blue', 'orange', 'red'];
   ...
   * @param array $marker
   */
  private function createMarker(array $marker): void
  {
    ...
    $this->addMarker(array_values($marker['coordinates']), $marker['animation'] ?? false, $marker['title'] ?? null);
